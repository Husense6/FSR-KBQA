...
from src.fsr.reason import llm_reason_grailqa
...
def run_reason(args):
    ...
    for item in tqdm(data):
        ...
        kb_knowledge = []
        for k in item['kb_knowledge']:
            k = json.loads(k)
            kb_knowledge.append(k['contents'])
    ...
    for index, (question, kopl, knowledges) in tqdm(enumerate(zip(questions, kopls, knowledge_snippets))):
        template, result = llm_reason_grailqa(question=question, kopl=kopl, llm_name=args.llm_name, reason_flag=args.reason_flag, knowledges=knowledges)
