...
Ground_truth_dataset_dir = 'data/GrailQA/GarilQA_all_llmkopl_knowledge.json'
Models_output_dir = 'outputs/GrailQA/llama3.1-70b-instruct-q4_K_M_iag.json'  # Replace with your output path
Dataset_aliases_dir = 'data/GrailQA/GrailQA_aliase_data6763.json'
...
with open(Models_output_dir, "r",encoding='utf-8') as file:
    _data = json.load(file)
    for gen, ground in zip(_data, questions_and_answers):
        if gen['question'] != ground['question']:
            print(gen['question'], ground['question'])
            print('question not match')
            raise ValueError
        # models_answers.append(gen['inference'])
        models_answers.append(gen['generate_answer'])
...
questions_and_answers = questions_and_answers[:len(models_answers)]
