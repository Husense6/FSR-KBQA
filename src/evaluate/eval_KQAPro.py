...
Ground_truth_dataset_dir = 'data/KQAPro/KQAPro_all_question_with_label_val.json'
Models_output_dir = 'outputs/KQAPro/70b/iag.json'
Dataset_aliases_dir = 'data/KQAPro/KQAPro_aliase_data106173.json'
...
with open(Ground_truth_dataset_dir, "r", encoding='utf-8') as file:
    ...
    questions_and_answers = [{'question': line['question'], 'answer': line['ans'], 'type_label': line['type_label'], 'type_label_ctm': line['type_label_ctm']} for line in _data]
...
with open(Models_output_dir, 'r', encoding='utf-8') as file:
    ...
    for line in json_data:
        ans = line['generate_answer'].rstrip().lstrip()
        if line['question'].rstrip().lstrip()!=questions_and_answers[num]['question'].rstrip().lstrip():
            print("An anomaly occurred when verifying the consistency of the question.")
            print("Question id: {}:\nQuestion input to the model:{}\nOriginal problem:{}".format(num,line['question'].rstrip().lstrip(),questions_and_answers[num]['question']))
            time.sleep(5)
        assert line['question'].rstrip().lstrip() == questions_and_answers[num]['question'].rstrip().lstrip()
        num += 1
...
def display_labeled_results(ans_type_count, ans_type_right, que_type_count, que_type_right):
    ...
    id2que_type = ['Multi-hop', 'Qualifier', 'Comparison', 'Logical', 'Count', 'Verify']
    ...
    que_data = {
        "question type": [id2que_type[i] for i in range(0, 6)],
        "EM Score": que_type_acc,
        "num": ["{}/{}".format(que_type_right[i], que_type_count[i]) for i in range(0, 6)]
    }
...
def evaluation():
    ...
    que_type_count = [0, 0, 0, 0, 0, 0]
    que_type_right = [0, 0, 0, 0, 0, 0]
    flag_list = []

    for i in tqdm(range(len(models_answers))):
        # if i < 11684:
        #     continue
        question = questions_and_answers[i]['question']
        answer = questions_and_answers[i]['answer']
        models_answer = models_answers[i].lower()
        label = questions_and_answers[i]['type_label_ctm']
        flag = 0
